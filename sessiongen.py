...
Espro = Client("Espro", api_id=API_ID, api_hash=API_HASH, in_memory=True)


async def main():
    await Espro.start()
    sess = await Espro.export_session_string()
    txt = f"Here is your Pyrogram {ver} String Session\n\n<code>{sess}</code>\n\nDon't share it with anyone.\nDon't forget to join @EsproAssociation"
    ok = await Espro.send_message("me", txt)
