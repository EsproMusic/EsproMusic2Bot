...
from EsproMusic.Helpers.active import add_active_chat, is_active_chat, stream_on
from EsproMusic.Helpers.downloaders import audio_dl
from EsproMusic.Helpers.errors import DurationLimitError
from EsproMusic.Helpers.gets import get_file_name, get_url
from EsproMusic.Helpers.inline import buttons
from EsproMusic.Helpers.queue import put
from EsproMusic.Helpers.thumbnails import gen_qthumb, gen_thumb
...
async def play(_, message: Message):
    Espro = await message.reply_text("» ᴘʀᴏᴄᴇssɪɴɢ, ᴘʟᴇᴀsᴇ ᴡᴀɪᴛ...")
    try:
    ...
    try:
        ...
        try:
            ...
            return await Espro.edit_text(
                f"ɪɴᴠɪᴛᴇ {BOT_NAME} ᴀssɪsᴛᴀɴᴛ ᴛᴏ {message.chat.title}.\n\n**ʀᴇᴀsᴏɴ :** `{ex}`"
            )
        try:
            await app2.resolve_peer(invitelink)
        except:
    ...
    if audio:
        ...
        try:
            ...
            return await Espro.edit_text(f"sᴏᴍᴇᴛʜɪɴɢ ᴡᴇɴᴛ ᴡʀᴏɴɢ\n\n**ᴇʀʀᴏʀ :** `{e}`")
        ...
        if len(message.command) < 2:
            return await Espro.edit_text("» ᴡʜᴀᴛ ᴅᴏ ʏᴏᴜ ᴡᴀɴɴᴀ ᴘʟᴀʏ ʙᴀʙʏ ?")
        await Espro.edit_text("🔎")
        ...
        try:
            ...
            LOGGER.error(str(e))
            return await Espro.edit("» ᴘʀᴏᴄᴇss ᴏ̨ᴜᴇʀʏ, ᴛʀʏ ᴘʟᴀʏɪɴɢ ᴀɢᴀɪɴ...")

        if (dur / 60) > DURATION_LIMIT:
            return await Espro.edit(
                f"» sᴏʀʀʏ ʙᴀʙʏ, ᴛʀᴀᴄᴋ ʟᴏɴɢᴇʀ ᴛʜᴀɴ  {DURATION_LIMIT} ᴍɪɴᴜᴛᴇs ᴀʀᴇ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ ᴘʟᴀʏ ᴏɴ {BOT_NAME}."
            )
        file_path = audio_dl(url)
    ...
    if await is_active_chat(message.chat.id):
        ...
        position = len(Esprodb.get(message.chat.id))
