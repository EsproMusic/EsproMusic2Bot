...
from EsproMusic.Helpers import (
    _clear_,
    admin_check_cb,
    gen_thumb,
    is_streaming,
...
from EsproMusic.Helpers.dossier import *
from EsproMusic.Helpers.inline import (
    buttons,
    close_key,
    help_back,
    helpmenu,
...
async def admin_cbs(_, query: CallbackQuery):
    ...
    if data == "resume_cb":
        ...
        get = Esprodb.get(query.message.chat.id)
        if not get:
            try:
                await _clear_(query.message.chat.id)
                await pytgcalls.leave_group_call(query.message.chat.id)
...
@app.on_callback_query(filters.regex("Espro_help"))
...
@app.on_callback_query(filters.regex("Espro_cb"))
...
@app.on_callback_query(filters.regex("Espro_home"))
async def home_Espro(_, query: CallbackQuery):
    try:
        await query.answer()
    except:
        pass
